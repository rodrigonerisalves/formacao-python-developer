...
with Session(engine) as session:
    digo = User(
        name="JOAO",
        fullname="JOAO1",
        address=[Address(email_address="joaoum@hotmail.com")]
    )

    maio = User(
        name="maria2",
        fullname="Maria Julia",
        address=[Address(email_address="mjulidois@hotmail.com")]
    )
    ...
    #enviar para db
    session.add_all([digo, maio, jose])

    session.commit()


stmt = select(User).where(User.name.in_(["digo", 'maio']))
